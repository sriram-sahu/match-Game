...
import MatchGameResult from './component/MatchGameResult'
...
class App extends Component {
  ...
  timer = () => {
    const {seconds, score, maxScore} = this.state
    if (seconds !== 0 && score < maxScore) {
      this.setState(prevState => ({seconds: prevState.seconds - 1}))
    } else {
      this.clearTimer()
      this.setState(prevState => ({isGameProgress: !prevState.isGameProgress}))
    }
  }

  getRandomImageIndex = () => {
    const randomIndex = Math.floor(Math.random() * imagesList.length)
    this.setState({imageItem: imagesList[randomIndex]})
  }
  ...
  render() {
    const {score, seconds, isGameProgress, activeTabId, imageItem} = this.state

    const filteredImagesList = this.getFilteredImagesList()
    this.checkSecondsNotZero()
    ...
    return (
      <div className="app-container">
        ...
        <div>
          {isGameProgress ? (
            <ul className="sub-app">
              <li key={imageUrl} className="list-styling">
                <img src={imageUrl} alt="match" className="random" />
              </li>
              <li>
                <ul className="list-cont">
                  {tabsList.map(eachItem => (
                    <TabItem
                      ...
                      isActiveTab={eachItem.tabId === activeTabId}
                    />
                  ))}
                </ul>
              </li>
              ...
              <li>
                <ul className="thumbnail-cont">
                  {filteredImagesList.map(eachItem => (
                    <ThumbnailItem
                      key={eachItem.thumbnailUrl}
                      thumbnailItem={eachItem}
                      updateMatchCount={this.matchImageAndThumbnail}
                    />
                  ))}
                </ul>
              </li>
            ...
            <ul className="sub-app">
              <MatchGameResult
                score={score}
                updateProgressStatus={this.updateGameProgressStatus}
              />
            </ul>
          )}
        </div>
